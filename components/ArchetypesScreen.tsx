...
const ArchetypesScreen: React.FC<ArchetypesScreenProps> = ({ customArchetypes, onShare }) => {
  ...
  const filteredArchetypes = useMemo(() => {
    // Force array creation from constants to ensure rendering
    const defaults = Object.keys(ARCHETYPE_INFO).map(name => ({
      ...ARCHETYPE_INFO[name],
      name
    }));
      ...
      {filteredArchetypes.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredArchetypes.map((arch, idx) => {
            ...
            return (
              <div key={`${arch.name}-${idx}`} className={`rune-border p-6 bg-black/60 backdrop-blur group hover:bg-red-900/5 transition-all flex flex-col ${isCustom ? 'border-gold/40 shadow-[0_0_20px_rgba(161,98,7,0.1)]' : 'border-red-900/30'}`}>
        ...
        <div className="py-24 text-center border-2 border-dashed border-red-900/20 rounded-lg">
          <p className="text-gray-500 font-cinzel italic text-lg uppercase tracking-widest opacity-40">No records found in the archive.</p>
        </div>
      )}
    </div>
  );
