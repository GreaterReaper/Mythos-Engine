...
const SpellsScreen: React.FC<SpellsScreenProps> = ({ mentors, playerCharacters, customArchetypes }) => {
  const [selectedArchetype, setSelectedArchetype] = useState<string | null>(null);

  const allArchetypesWithSpells = new Set<string>();
  
  Object.entries(ARCHETYPE_INFO).forEach(([key, info]) => {
    if (info.spells && info.spells.length > 0) allArchetypesWithSpells.add(key);
  });

  customArchetypes.forEach(ca => {
    if (ca.spells && ca.spells.length > 0) allArchetypesWithSpells.add(ca.name);
  });

  playerCharacters.forEach(c => {
    if (c.spells && c.spells.length > 0) allArchetypesWithSpells.add(c.archetype);
  });
  ...
  const activeArchetype = selectedArchetype || Array.from(allArchetypesWithSpells)[0];
  ...
  return (
    <div className="space-y-8 pb-20 max-w-5xl mx-auto">
      ...
      <div className="flex gap-2 overflow-x-auto pb-4 custom-scrollbar no-scrollbar-md">
        {Array.from(allArchetypesWithSpells).map(arch => {
      ...
      {activeArchetype ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
          ...
          {getSpellsForArchetype(activeArchetype).length === 0 && (
            <div className="col-span-full py-20 text-center border border-dashed border-red-900/20 rounded-lg">
              <p className="text-gray-600 font-cinzel italic text-xs">No spells recorded for this path in the aetheric record.</p>
            </div>
          )}
        </div>
      ) : (
