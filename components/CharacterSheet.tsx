...
const CharacterSheet: React.FC<CharacterSheetProps> = ({ character, onUpdate, isMentor }) => {
  ...
  const toggleStatus = (effect: StatusEffect) => {
    if (!onUpdate) return;
    const nextStatuses = character.activeStatuses.includes(effect)
  ...
  const handleManifestSpell = (spell: Ability) => {
    ...
    if (arcaneMemoryActive) {
      if (arcaneMemoryUsed) { alert("Thy Arcane Memory is spent for this cycle."); return; }
    ...
    if (availableLevels.length === 0) { alert(`Thy wells of level ${baseLevel} or higher are dry.`); return; }
  ...
  const handleManifestNewLore = async () => {
    ...
    } catch (e) { console.error("Lore weaving failed:", e); } finally { setIsWeaving(false); }
  };
  ...
  const comparisonData = useMemo(() => {
    ...
    const allKeys = new Set([...Object.keys(newItem.stats || {}), ...Object.keys(equipped.stats || {})]);
    allKeys.forEach(key => {
  ...
  const getStatusColor = (status: StatusEffect) => {
    switch (status) {
      case 'Poisoned': return 'text-emerald-500 border-emerald-900/40 bg-emerald-900/10';
      case 'Blinded': return 'text-gray-400 border-gray-700 bg-gray-900/20';
      case 'Stunned': return 'text-yellow-400 border-yellow-900/40 bg-yellow-900/10';
      case 'Paralyzed': return 'text-blue-400 border-blue-900/40 bg-blue-900/10';
      case 'Frightened': return 'text-purple-500 border-purple-900/40 bg-purple-900/10';
      case 'Bleeding': return 'text-red-500 border-red-900/40 bg-red-900/10';
    ...
    <div className="rune-border bg-black/90 backdrop-blur-xl overflow-hidden flex flex-col h-full max-h-[90vh] shadow-2xl">
      ...
      <div className="flex border-b border-red-900/30 text-[10px] font-cinzel overflow-x-auto whitespace-nowrap bg-black/40 no-scrollbar">
        {['Stats', 'Abilities', 'Inventory', 'Lore'].map(t => (
          <button key={t} onClick={() => setActiveTab(t as any)} className={`flex-1 min-w-[80px] py-3 md:py-4 transition-all uppercase tracking-widest ${activeTab === t ? 'bg-red-900/20 text-gold border-b-2 border-gold font-bold' : 'text-gray-500 hover:text-gray-300'}`}>{t}</button>
        ))}
      </div>
        ...
        {activeTab === 'Stats' && (
          <div className="space-y-8 pb-4">
            ...
            <div className="space-y-3">
               <h3 className="text-[10px] font-cinzel text-red-700 uppercase border-b border-red-900/20 pb-2 font-black tracking-[0.2em]">Aetheric Blights (Conditions)</h3>
                   ...
                   return (
                     <button 
                       ...
                       className={`px-3 py-1.5 rounded-sm border text-[8px] font-black uppercase tracking-widest transition-all ${
                         isActive 
                           ? `${getStatusColor(status)} shadow-[0_0_10px_currentColor]` 
                           : 'border-white/10 text-white/20 hover:text-white/40'
                       }`}
                     >
                       {status}
                     </button>
               ...
               {character.activeStatuses.length === 0 && (
                 <p className="text-[9px] text-gray-600 italic">No blights currently binding this soul.</p>
               )}
            </div>

            {/* Vault (Currency) */}
            <div className="space-y-3">
               <h3 className="text-[10px] font-cinzel text-gold uppercase border-b border-gold/20 pb-2 font-black tracking-[0.2em]">The Vault</h3>
            ...
            {isCaster && (
              <div className="space-y-3 animate-in fade-in slide-in-from-top-4 duration-700">
                ...
                <SpellSlotManager maxSlots={character.maxSpellSlots!} currentSlots={character.spellSlots || {}} onUseSlot={handleUseSlot} onRestoreAll={handleRestoreSlots} isReadOnly={!!isMentor} />
              </div>
            )}
                ...
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex flex-col items-end">
                    ...
                    {a.type === 'Spell' && !isMentor && (
                      <button onClick={() => handleManifestSpell(a)} className={`mt-2 px-3 py-1 text-[8px] font-black uppercase tracking-widest transition-all rounded-sm border ${a.name === 'Exequy' ? 'bg-red-950 text-red-500 border-red-800 hover:bg-red-900 shadow-[0_0_10px_#991b1b]' : 'bg-gold/10 text-gold border-gold/40 hover:bg-gold hover:text-black'}`}>Manifest {a.name === 'Exequy' ? 'Oblivion' : (arcaneMemoryActive ? 'via Memory' : 'Aether')}</button>
                    )}
                  </div>
                </div>
          ...
          <div className="space-y-6">
            {!isMentor && (
              <div className="flex justify-between items-center mb-2">
                ...
                <button onClick={handleAutoEquip} className="text-[10px] font-cinzel text-gold bg-gold/10 border border-gold/40 px-3 py-1.5 hover:bg-gold/20 transition-all flex items-center gap-2"><svg className="w-3 h-3 animate-pulse" fill="currentColor" viewBox="0 0 20 20"><path d="M11 3a1 1 0 10-2 0v1a1 1 0 102 0V3zM15.657 5.757a1 1 0 00-1.414-1.414l-.707.707a1 1 0 001.414 1.414l.707-.707zM18 10a1 1 0 01-1 1h-1a1 1 0 110-2h1a1 1 0 011 1zM5.05 6.464A1 1 0 106.464 5.05l-.707-.707a1 1 0 00-1.414 1.414l.707.707zM5 10a1 1 0 01-1 1H3a1 1 0 110-2h1a1 1 0 011 1zM8 16v-1a1 1 0 112 0v1a1 1 0 11-2 0zM13.536 14.95a1 1 0 011.414 0l.707.707a1 1 0 11-1.414 1.414l-.707-.707a1 1 0 010-1.414zM15 10a5 5 0 11-10 0 5 5 0 0110 0z" /></svg>Equip All</button>
              </div>
            )}
            {comparisonData && (
              <div className="rune-border p-4 bg-amber-900/10 border-gold/40 animate-in slide-in-from-top-4 duration-500 space-y-4">
                 <div className="flex justify-between items-center border-b border-gold/20 pb-2"><h4 className="text-[10px] font-cinzel text-gold uppercase font-black tracking-widest">Soul Resonance Comparison</h4><button onClick={() => setComparingItemId(null)} className="text-gold/40 hover:text-white text-xs">×</button></div>
                 <div className="flex gap-4">
                    <div className="flex-1 space-y-2">
                       <p className="text-[9px] text-gray-500 uppercase font-black">Equipped: {comparisonData.equipped?.name || 'Nothing'}</p>
                       <div className="grid grid-cols-1 gap-1">{Object.entries(comparisonData.deltas).map(([key, delta]) => {
                         if (key === 'damage') return (<div key={key} className="flex justify-between text-[10px] py-1 border-b border-white/5"><span className="uppercase text-gray-400">{key}</span><span className="text-white font-mono">{delta}</span></div>);
                         const val = delta as number; return (<div key={key} className="flex justify-between text-[10px] py-1 border-b border-white/5"><span className="uppercase text-gray-400">{key}</span><span className={`font-black font-mono ${val > 0 ? 'text-green-500' : val < 0 ? 'text-red-500' : 'text-gray-500'}`}>{val > 0 ? `+${val}` : val}</span></div>);
                       })}</div>
                    </div>
                    <div className="w-1/3 flex flex-col justify-center items-center bg-black/40 border border-gold/20 p-4 rounded-sm"><div className={`w-12 h-12 border-2 ${comparisonData.newItem.rarity === 'Legendary' ? 'border-orange-500' : comparisonData.newItem.rarity === 'Epic' ? 'border-purple-500' : 'border-gold/40'} bg-black flex items-center justify-center font-cinzel text-xl font-black text-white mb-2 shadow-[0_0_15px_rgba(0,0,0,0.5)]`}>{comparisonData.newItem.name[0]}</div><button onClick={() => toggleEquip(comparisonData.newItem.id)} className="w-full py-2 bg-gold text-black font-cinzel text-[10px] font-black uppercase tracking-tighter hover:bg-white transition-all shadow-lg">Bind Now</button></div>
                 </div>
              </div>
            )}
            <div className="space-y-3">
              {character.inventory.map((item) => {
                const isEquipped = character.equippedIds?.includes(item.id); const isComparing = comparingItemId === item.id; const canCompare = !isEquipped && (item.type === 'Weapon' || item.type === 'Armor');
                return (
                  <div key={item.id} onClick={() => canCompare && setComparingItemId(item.id)} className={`p-3 border transition-all flex justify-between items-center group rounded-sm relative ${isEquipped ? 'border-gold bg-gold/5' : isComparing ? 'border-gold shadow-[0_0_10px_#d4af37]' : 'border-red-900/10 bg-black/40 hover:border-gold/20 cursor-pointer'}`}>
                    <div className="flex gap-3 items-center min-w-0"><div className={`w-10 h-10 border-2 flex items-center justify-center text-sm font-black shrink-0 rounded-sm ${item.rarity === 'Legendary' ? 'border-orange-500 text-orange-500 shadow-[0_0_10px_rgba(249,115,22,0.3)]' : item.rarity === 'Epic' ? 'border-purple-500 text-purple-500' : item.rarity === 'Rare' ? 'border-blue-500 text-blue-500' : 'border-red-900/30 text-red-900'}`}>{item.name[0]}</div><div className="min-w-0"><p className="text-[11px] font-cinzel text-gold truncate uppercase font-black">{item.name}</p><p className="text-[9px] text-gray-500 uppercase font-bold tracking-tighter">{item.rarity} • {item.type}</p></div></div>
                    <div className="flex items-center gap-3">{canCompare && !isComparing && (<span className="text-[8px] text-gold/40 uppercase font-black group-hover:text-gold transition-colors">Compare</span>)}<button onClick={(e) => { e.stopPropagation(); toggleEquip(item.id); }} disabled={isMentor} className={`px-4 py-1.5 text-[10px] font-cinzel uppercase border-2 transition-all font-black ${isEquipped ? 'bg-gold text-black border-gold shadow-lg' : 'border-red-900 text-red-900 hover:border-gold hover:text-gold'} ${isMentor ? 'opacity-50 cursor-not-allowed' : ''}`}>{isEquipped ? 'Equipped' : 'Equip'}</button></div>
                  </div>
                );
              })}
            </div>
            ...
            <div className="flex justify-between items-center mb-2">
               ...
               {!isMentor && (
                 <div className="flex gap-2">
                   {isEditingLore ? (
                     <><button onClick={handleManifestNewLore} disabled={isWeaving} className="text-[9px] font-cinzel text-gold bg-gold/10 border border-gold/40 px-2 py-1 hover:bg-gold/20 transition-all flex items-center gap-1 disabled:opacity-30"><svg className={`w-3 h-3 ${isWeaving ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>Aetheric Re-Weave</button><button onClick={saveLore} className="text-[9px] font-cinzel text-green-500 bg-green-500/10 border border-green-500/40 px-3 py-1 hover:bg-green-500/20 transition-all">SAVE</button><button onClick={() => { setIsEditingLore(false); setEditedBio(character.biography || ''); setEditedDesc(character.description || ''); }} className="text-[9px] font-cinzel text-red-500 bg-red-500/10 border border-red-500/40 px-3 py-1 hover:bg-red-500/20 transition-all">CANCEL</button></>
                   ) : (
