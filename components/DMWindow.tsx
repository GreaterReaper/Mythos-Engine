...
const VitalityMonitor: React.FC<{ character: Character, isActive: boolean, onClick: () => void }> = ({ character, isActive, onClick }) => {
  ...
  return (
    <div onClick={onClick} className={`p-2.5 rounded transition-all cursor-pointer relative overflow-hidden ${isActive ? 'bg-gold/10 border border-gold/40' : 'hover:bg-emerald-900/5 border border-transparent'}`}>
      ...
      <div className="h-1.5 w-full bg-black rounded-full overflow-hidden border border-white/5 relative z-10">
        <div className={`h-full transition-all duration-700 ${character.currentHp <= 0 ? 'bg-red-900' : 'bg-emerald-600'}`} style={{ width: `${percent}%` }} />
      </div>
      {hpChange && (
...
const DMWindow: React.FC<DMWindowProps> = ({ 
  ...
  const renderContent = (content: string) => {
    // Regex to find dice roll blocks: [🎲 ... ] or [💥 ... ] or [Roll: ...]
    const rollRegex = /(\[.*?\])/g;
      ...
      <div className="leading-relaxed whitespace-pre-wrap font-medium text-sm md:text-base">
        {parts.map((part, i) => {
          if (part.match(rollRegex)) {
            return (
              <span key={i} className="inline-block px-1.5 py-0.5 bg-emerald-950/40 border border-emerald-500/30 text-emerald-400 font-mono text-[11px] md:text-xs font-black rounded mx-1 shadow-[0_0_10px_rgba(16,185,129,0.1)]">
                {part}
              </span>
            );
          }
  ...
  return (
    <div className="flex flex-col h-full w-full overflow-hidden bg-[#0c0a09]">
      <div className="md:hidden flex overflow-x-auto no-scrollbar gap-2 p-2 bg-black border-b border-emerald-900/40 shrink-0 z-30 shadow-lg">
         {characters.map(char => (
           <div key={char.id} onClick={() => onSelectActiveCharacter(char.id)} className={`min-w-[100px] p-2 rounded border transition-all ${char.id === activeCharacter?.id ? 'bg-gold/10 border-gold/60' : 'bg-black border-emerald-900/20'}`}>
      ...
      <div className="flex flex-1 min-h-0 relative">
        <div className="flex flex-col flex-1 min-w-0">
          <div className="px-4 py-3 border-b-2 border-emerald-900/60 flex justify-between items-center bg-black/80 backdrop-blur shrink-0 z-20">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-2"><div className={`w-1.5 h-1.5 rounded-full ${isLoading ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500'}`} /><p className="text-[8px] text-emerald-500/60 font-black uppercase tracking-tighter">{isLoading ? 'Resonance Loading...' : 'Aether Connected'}</p></div>
              </div>
            </div>
          </div>
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 md:px-12 md:py-8 space-y-6 custom-scrollbar bg-[#0c0a09] relative">
            ...
            {isLoading && <div className="flex justify-start animate-in fade-in relative z-10"><div className="bg-black border-l-4 border-emerald-900 p-5 shadow-xl max-w-[85%] rounded-r-xl"><p className="text-[9px] font-cinzel text-emerald-500 mb-2 font-black uppercase">Weaving Fate...</p><div className="flex gap-2"><div className="w-2 h-2 bg-emerald-900 rounded-full animate-bounce" /><div className="w-2 h-2 bg-emerald-700 rounded-full animate-bounce delay-100" /><div className="w-2 h-2 bg-emerald-500 rounded-full animate-bounce delay-200" /></div></div></div>}
          </div>
          <div className="shrink-0 bg-black border-t-2 border-emerald-900/40 p-4 pb-20 md:pb-4 z-20">
            <div className="flex gap-3 items-end max-w-5xl mx-auto w-full">
              <textarea value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && !e.shiftKey && (e.preventDefault(), handleSend())} placeholder={isDying ? "THOU ART UNCONSCIOUS..." : "VOICE THY WILL..."} disabled={isDying || isLoading} className="flex-1 bg-[#1c1917] border-2 border-emerald-900/20 p-3 text-gold text-sm md:text-base h-24 md:h-20 focus:border-gold outline-none resize-none rounded-lg transition-all" />
        ...
        <div className="hidden md:flex flex-col w-80 bg-[#0c0a09] border-l-2 border-emerald-900/30 overflow-hidden shrink-0 shadow-2xl">
          <div className="p-5 border-b border-emerald-900/20 bg-emerald-900/5 shrink-0">
             <div className="flex justify-between items-center mb-4"><h4 className="text-[10px] font-cinzel text-emerald-500 font-black uppercase tracking-widest">Fellowship Monitor</h4><div className={`w-2 h-2 rounded-full ${isLoading ? 'bg-amber-500 animate-pulse' : 'bg-emerald-500'}`} /></div>
          ...
          <div className="p-5 border-b border-emerald-900/20 bg-black/40 h-40 flex flex-col shrink-0">
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pr-2">
                {campaign.history.filter(m => m.role === 'system' && m.content.startsWith('SCRIBE_LOG:')).map((log, i) => <div key={i} className="text-[9px] font-mono text-emerald-500/80 leading-tight">✧ {log.content.replace('SCRIBE_LOG:', '')}</div>)}
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-5 space-y-8 bg-black/20">
             {activeCharacter?.spellSlots && activeCharacter.maxSpellSlots && (
               <div><h4 className="text-[10px] font-cinzel text-amber-500 font-black uppercase mb-4 tracking-widest border-b border-amber-900/20 pb-1">Aetheric Wells</h4><SpellSlotManager currentSlots={activeCharacter.spellSlots} maxSlots={activeCharacter.maxSpellSlots} onUseSlot={() => {}} onRestoreAll={() => {}} isReadOnly /></div>
             )}
             <div>
                ...
                <div className="space-y-3">
                  {usableManifestations.length > 0 ? usableManifestations.map((spell, i) => (
                  ...
                  )) : <p className="text-[9px] text-gray-700 italic text-center py-4">No spells manifested.</p>}
                </div>
             </div>
          </div>
        </div>
