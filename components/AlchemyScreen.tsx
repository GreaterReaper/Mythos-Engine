  ...
  return (
    <div className="space-y-8 pb-20 px-2">
      <div className="border-b border-emerald-900 pb-4">
        <h2 className="text-4xl font-cinzel text-emerald-500 font-black">The Alchemy Lab</h2>
        <p className="text-gray-500 italic text-sm">"Potions of life and flasks of ruin. Reagents Manifested from the world's blood."</p>
      </div>
      ...
      {isAlchemistPresent && (
        <div className="rune-border p-6 bg-emerald-950/10 border-emerald-500/30 space-y-6 animate-in slide-in-from-top-4 duration-700">
           <div className="flex items-center gap-3 border-b border-emerald-500/20 pb-3">
             <div className="w-10 h-10 rounded-full bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center shadow-[0_0_15px_rgba(16,185,129,0.3)]">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
             {recipes.map((recipe, idx) => (
               <div key={idx} className="p-4 bg-black/60 border border-emerald-900/30 hover:border-emerald-500/50 transition-all rounded-sm group">
                 <p className="text-emerald-500 font-cinzel font-bold text-sm uppercase group-hover:text-gold transition-colors">{recipe.name}</p>
                 <p className="text-[10px] text-gray-500 italic mb-3 leading-relaxed">{recipe.description}</p>
      ...
      {consumables.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {consumables.map(item => (
            <div key={item.id} className="relative group rune-border p-5 bg-black/60 border-emerald-900/30 hover:border-emerald-500/50 transition-all flex flex-col space-y-4 shadow-xl">
              <div className="flex justify-between items-start border-b border-emerald-900/30 pb-2">
                <div className="min-w-0">
                  <h4 className="font-cinzel text-lg text-emerald-400 truncate font-bold group-hover:text-gold transition-colors">{item.name}</h4>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button onClick={() => onShare(item)} className="text-emerald-500/40 hover:text-emerald-500 transition-colors" title="Share Formula">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" /></svg>
                  </button>
                  {!item.id.startsWith('start-') && (
                    <button onClick={() => handleDelete(item.id)} className="text-emerald-900 hover:text-emerald-500 transition-colors text-xl font-bold" title="Dissolve">×</button>
                  )}
                </div>
              </div>
              
              <div className="flex gap-4 items-center">
                 <div className="w-12 h-12 bg-emerald-900/20 border border-emerald-500/30 flex items-center justify-center font-cinzel text-xl text-emerald-500 rounded-sm shrink-0 shadow-inner">
                   {item.name[0]}
                 </div>
                 <p className="text-xs text-gray-400 italic leading-relaxed flex-1 line-clamp-3 font-medium">"{item.description}"</p>
              </div>
              
              {item.stats && Object.keys(item.stats).length > 0 && (
                <div className="grid grid-cols-1 gap-1 pt-2">
                  {Object.entries(item.stats).map(([key, val]) => (
                    <div key={key} className="flex justify-between text-[10px] border border-emerald-900/20 px-2 py-1 bg-emerald-900/5 rounded-sm">
                      <span className="uppercase text-emerald-900 font-cinzel font-black tracking-widest">{key}</span>
        ...
        <div className="py-32 text-center border-2 border-dashed border-emerald-900/20 bg-black/40 rounded-lg">
           <p className="font-cinzel text-gray-600 uppercase tracking-widest italic font-bold">No tonics or reagents manifested in thy lab.</p>
           <p className="text-[10px] text-gray-700 mt-2 font-black uppercase tracking-tighter">Trade with merchants or carve parts from the fallen.</p>
        </div>
      )}
    </div>
  );
